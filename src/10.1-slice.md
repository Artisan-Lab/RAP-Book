...
Like Kani (`#[cfg_attr(kani, ...)]`) and Flux (`#[cfg(flux)]`), RAPx annotations use `cfg_attr` so they only activate when the tool runs — `register_tool` is injected via `RUSTFLAGS`, never hard-coded in source:
...
1. **Entry (`init_parameters`)** — for a `&[T]` / `&mut [T]` parameter `self`, the VM establishes the slice's validity as symbolic state:
   ...
   - the data allocation itself carries `align = align_of(T)`, `size = len * sizeof_T`, `element_ty = T`, where `sizeof_T` is the shared symbolic element size (≥ 1 for a generic `T`), used both for the data allocation's byte size and for the invariant's `size_of(T)` bound.
