...
On top of these, the `&[T]` receiver `self` carries a slice type invariant (`NonNull` + `Align` + `ValidNum(size_of(T)*len <= isize::MAX)` + `Allocated` + `Init`) that `init_parameters` establishes at entry and that is re-proved at the function's `return` block.
