...
1. **Entry (`init_parameters`)** — for a `&[T]` / `&mut [T]` parameter `self`, the VM establishes the slice's validity as symbolic state:
   ...
   - entry-fixed, not re-proved: the length bounds `0 <= len` (usize non-negativity) and `len * size_of(T) <= isize::MAX` — both fixed at entry because the reference's address and length are immutable, and the size bound is re-enforced wherever a new slice is created (`from_raw_parts`);
...
`NonNull` and `Align` hold for every slice — even an empty one, whose dangling data pointer (`NonNull::dangling`) is still non-null and properly aligned (Rust requires alignment for size-0 access). `Allocated` and `Init` hold only for non-empty slices, so they are wrapped in `any(len == 0, …)`. The length bounds (`0 <= len`, `len * size_of(T) <= isize::MAX`) are slice invariants too, but not part of the *re-proved* set: they are fixed at entry (the reference's address and length are immutable, so no body operation can change them). `0 <= len` is just the usize non-negativity and is never re-checked; the size bound `len * size_of(T) <= isize::MAX` is re-enforced where a new slice is introduced, at `from_raw_parts` (`ValidNum(size_of(T) * len <= isize::MAX)`). `Align` is likewise re-checked at `&*ptr` (`Ptr2Ref = Init + Align + Alias`).
...
entry   _1 = self (&[T]), _2 = index (I)
          ...
          init_parameters:
            _1: term = data base, prov = (data alloc, 0), flags { non_null, aligned, init }
               ...
               entry-fixed, not re-proved:
                 0 ≤ len(_1)  (usize non-negativity),  len(_1)·size_of(T) ≤ isize::MAX  (re-enforced at `from_raw_parts`)
