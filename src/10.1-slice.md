...
1. **Entry (`init_parameters`)** — for a `&[T]` / `&mut [T]` parameter `self`, the VM establishes the slice's full validity (its built-in invariant) as symbolic state:
   - re-proved at return — `NonNull` (flag `non_null`), `Init` (flag `init` + mark `initialized = true`), `Allocated` (the data allocation itself);
   - entry-fixed, not re-proved — `Align` (flag `aligned`) and the length bounds `0 <= len`, `len * size_of(T) <= isize::MAX`;
...
#[rapx::invariant(Allocated(self, T, len(self)))]
```

Of the five parts of the slice invariant, only `NonNull`/`Init`/`Allocated` are re-proved — they are what unsafe code in the body can still break (a raw write or `drop_in_place` can de-initialize the elements (`Init`) or free the memory (`Allocated`)). `Align` and the length bounds are fixed at entry (the reference's address and length are immutable), so they are not re-proved but enforced where a new pointer or length is introduced: `&*ptr` (`Ptr2Ref = Init + Align + Alias`) and `from_raw_parts` (`Align(data, T)` + `ValidNum(size_of(T) * len <= isize::MAX)`).
...
On top of these, the `&[T]` receiver `self` carries a built-in slice invariant (`NonNull` + `Init` + `Allocated`) that `init_parameters` establishes at entry and that is re-proved at every path endpoint.
...
entry   _1 = self (&[T]), _2 = index (I)
          ...
          init_parameters:
            _1: term = data base, prov = (data alloc, 0), flags { non_null, aligned, init }
               built-in slice invariant (RAPx-synthesized):
                 NonNull(_1) · Init(_1, T, len(_1)) · Allocated(_1, T, len(_1))   — re-proved at return
                 Align(_1, T) · 0 ≤ len(_1), len(_1)·size_of(T) ≤ isize::MAX      — entry-fixed (checked at `&*ptr` / `from_raw_parts`)
...
return                                                   [type-invariant]
          check  NonNull(_1), Init(_1, T, len), Allocated(_1, T, len)  ←  _1 unchanged   Proved
...
| ret | `return` | type-invariant: `NonNull` + `Init` + `Allocated` | — | ← `_1` invariant |
...
- (2) `from_raw_parts`'s `ValidPtr`/`InBound`/`ValidNum`: `len % N == 0` gives `len = new_len * N`, so the byte size `size_of([T; N]) * new_len` equals the slice's own size. `NonNull`/`Init`/`Allocated` are inherited from the slice invariant, while `Align`/`ValidNum`/`Alive`/`Alias` follow from the slice's full validity established at entry. The `cast` re-interprets `T` as `[T; N]`, layout-identical by construction.
