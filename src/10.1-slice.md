...
1. **Entry (`init_parameters`)** — for a `&[T]` / `&mut [T]` parameter `self`, the VM establishes the slice's validity as symbolic state:
   - the built-in slice invariant (re-proved at return): `NonNull` (flag `non_null`), `Align` (flag `aligned`), and `any(len(self) == 0, (Allocated, Init))` — the data allocation itself plus element initialization, required only for non-empty slices;
...
#[rapx::invariant(Align(self, T))]
#[rapx::invariant(any(ValidNum(len(self) == 0), (Allocated(self, T, len(self)), Init(self, T, len(self)))))]
```
...
On top of these, the `&[T]` receiver `self` carries a built-in slice invariant (`NonNull` + `Align` + `any(len == 0, (Allocated, Init))`) that `init_parameters` establishes at entry and that is re-proved at every path endpoint.
...
entry   _1 = self (&[T]), _2 = index (I)
          ...
          init_parameters:
            _1: term = data base, prov = (data alloc, 0), flags { non_null, aligned, init }
               built-in slice invariant (RAPx-synthesized; re-proved at return):
